...
uses
  ...
  IdIOHandler, IdIOHandlerSocket, IdSSLOpenSSL, DB, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, IdMessage, jpeg;
...
type
  TFrmConfEmail = class(TForm)
    ...
    procedure btn_selecao_membrosClick(Sender: TObject);
...
procedure TFrmConfEmail.btn_selecao_membrosClick(Sender: TObject);
begin
   if frm_lista_membro =nil then
  frm_lista_membro:=Tfrm_lista_membro.Create(Self);
   frm_lista_membro.show;
   edtContaEmail.Clear;
end;
